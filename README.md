...
its own right. Type `/mode:` and the palette lists all of them:

```
/mode:tdd              # a mode is just its name
/mode:tester
/mode:style:ship       # a style carries its axis
/mode:style:edu
```

The style spelling has the extra segment for a reason worth knowing before you write a contract of
your own. Claude Code namespaces a plugin's commands under the plugin, so everything this one ships
starts with `mode:`. Left alone, `ship` and `tdd` would sit side by side as `/mode:ship` and
`/mode:tdd` with nothing saying which slot each one fills. Putting `style` in the name of a style's
command file keeps the axis visible in the palette.

These are generated by `mode sync` from the contract folders, so writing a new contract gives it a
shortcut with no second step. They do exactly what the longer spelling does, and the hook treats
them as one code path, so nothing behaves differently depending on which you type.

**One caveat if you are on Windows.** A style's command file has a colon in its filename, which is
legal on macOS and Linux and illegal on Windows, so those five files will not check out there. If
that affects you, rename them to `style-ship.md` and so on, which gives `/mode:style-ship`.
...
working. `/mode` itself needs no shorthand, because the skill that answers to it is named after the
plugin.
